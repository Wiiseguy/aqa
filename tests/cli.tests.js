...
test('Verbose - via process.env', async t => {
    let result = await t.throwsAsync(
        async _ => await exec(`node cli tests/_self/should-fail`, { env: { AQA_VERBOSE: 'true' } })
    );
    let stdout = Color.strip(result.stdout);
...
test('Handle invalid exceptions', async t => {
    ...
    t.true(
        stdout.includes(
            'Error: [WARNING: aqa could not determine stack information, because the thrown object was not an Error.] Bye1'
        )
    );
    t.true(
        stdout.includes(
            'Error: [WARNING: aqa could not determine stack information, because the thrown object was not an Error.] Bye2'
        )
    );
});
...
test('Test TAP report - default', async t => {
    let result = await t.throwsAsync(
        async _ => await exec(`node cli tests/_self/success-fail`, { env: { AQA_REPORTER: 'tap' } })
    );
    let stdout = Color.strip(result.stdout);
    let expected =
        'TAP version 13\n' +
...
test('Test JUnit report - default', async t => {
    ...
    let result = await t.throwsAsync(
        async _ => await exec(`node cli tests/_self/before-fail`, { env: { AQA_REPORTER: 'junit' } })
    );
    let stdout = Color.strip(result.stdout);
    t.true(stdout.includes('FAILED:  "before"'));
    t.true(stdout.includes('SKIPPED:  x1'));
...
test('Test JUnit report - skip-file', async t => {
    ...
    t.true(
        report.includes('<testcase name="before - /tests/_self/skip-file.js" classname="/tests/_self/skip-file.js"')
    );
...
test('Test JUnit report - custom report dir', async t => {
    ...
    let result = await t.throwsAsync(
        async _ =>
            await exec(`node cli tests/_self/before-fail`, {
                env: { AQA_REPORTER: 'junit', AQA_REPORTER_OUTPUT_DIR: '.aqa-custom-output/' }
            })
    );
    let stdout = Color.strip(result.stdout);
    t.true(stdout.includes('FAILED:  "before"'));
